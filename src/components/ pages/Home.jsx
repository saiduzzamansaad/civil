...
import { 
  HardHat, Ruler, Box, Zap, ClipboardCheck, 
  ArrowRight, Plus, Search, RefreshCw,
  Calculator, Weight, BrickWall, Loader,
  Download, Trash2, Eye, ChevronDown, X,
  LandPlot, Factory, Hammer, Drill, Warehouse,
  Scaling, Gauge, KeyboardMusic, BrickWallFire , Cable, Refrigerator 
} from 'lucide-react';
...
const useDynamicTheme = () => {
  ...
  useEffect(() => {
    const themes = [
      { primary: 'from-blue-600 to-blue-800', secondary: 'from-emerald-600 to-teal-700', accent: 'from-purple-600 to-indigo-700' },
      { primary: 'from-amber-600 to-orange-700', secondary: 'from-rose-600 to-pink-700', accent: 'from-violet-600 to-fuchsia-700' },
      { primary: 'from-green-600 to-lime-700', secondary: 'from-cyan-600 to-sky-700', accent: 'from-indigo-600 to-blue-700' },
      { primary: 'from-red-600 to-rose-700', secondary: 'from-yellow-600 to-amber-700', accent: 'from-pink-600 to-rose-700' }
    ];
...
const Home = () => {
  ...
  const tools = useMemo(() => [
    ...
    },
    {
      ...
      description: "Design and analyze different types of foundations",
      path: "/foundation-designer",
      icon: <LandPlot size={24} className="text-indigo-400" />,
      bgColor: "from-indigo-600 to-blue-800",
      category: 'structural'
    },
    {
      title: "Beam Calculator",
      description: "Analyze and design reinforced concrete beams",
      path: "/beam-calculator",
      icon: <Refrigerator  size={24} className="text-cyan-400" />,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        >
          ...
          >
            <div className="flex items-center justify-center bg-white/80 backdrop-blur-md rounded-full px-6 py-3 shadow-sm border border-gray-200">
              <HardHat size={24} className="text-blue-600 mr-2" />
              <span className="text-blue-600 font-medium">Civil Engineering Suite PRO</span>
            </div>
          </motion.div>
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-4">
            <span 
              ...
              style={{
                backgroundImage: `linear-gradient(${gradientAngle}deg, #2563eb, #4f46e5, #7c3aed)`
              }}
            >
              Advanced Tools
            </span> for Modern Construction
          </h1>
          <p className="text-xl md:text-2xl text-gray-600 max-w-3xl mx-auto font-light">
            Precision calculation tools designed for civil engineers and construction professionals
          </p>
        </motion.div>
        ...
        >
          <div className={`bg-white/90 backdrop-blur-lg rounded-xl shadow-lg p-4 md:p-6 transition-all duration-300 ${isScrolled ? 'border border-gray-200' : ''}`}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center space-x-2">
                <div className="flex rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  <button
                    type="button"
                    className={`px-4 py-2 text-sm font-medium transition-colors flex items-center ${activeTab === 'all' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}
                  ...
                  <button
                    type="button"
                    className={`px-4 py-2 text-sm font-medium transition-colors flex items-center ${activeTab === 'structural' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}
                  ...
                  <button
                    type="button"
                    className={`px-4 py-2 text-sm font-medium transition-colors flex items-center ${activeTab === 'materials' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}
                    onClick={() => setActiveTab('materials')}
                  >
                    <BrickWall className="h-4 w-4 mr-1" />
                    Materials
                  </button>
                  <button
                    type="button"
                    className={`px-4 py-2 text-sm font-medium transition-colors flex items-center ${activeTab === 'utilities' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}
                  ...
                  >
                    ...
                    <div className="absolute top-4 right-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        tool.category === 'structural' ? 'bg-blue-100 text-blue-800' :
                        tool.category === 'materials' ? 'bg-emerald-100 text-emerald-800' :
                        'bg-purple-100 text-purple-800'
                      }`}>
                        {tool.category === 'structural' ? 'Structural' : 
                         tool.category === 'materials' ? 'Materials' : 'Utilities'}
                      </span>
                    </div>
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-white/90 font-light">{tool.description}</p>
                    </div>
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-2 md:mb-0">Recent Calculations</h2>
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">{calc.tool}</div>
                            <div className="text-xs text-gray-500 flex items-center">
                              {categoryIcons[calc.category]}
                              {calc.category === 'structural' ? 'Structural' : 
                               calc.category === 'materials' ? 'Materials' : 'Utilities'}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{calc.date}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{calc.parameters}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => setNewTool({...newTool, bgColor: e.target.value})}
                        >
                          <option value="from-blue-600 to-blue-800">Blue</option>
                          <option value="from-emerald-600 to-teal-700">Emerald</option>
                          <option value="from-purple-600 to-indigo-700">Purple</option>
                          <option value="from-amber-600 to-orange-700">Amber</option>
                          <option value="from-gray-600 to-gray-800">Gray</option>
                          <option value="from-red-600 to-red-800">Red</option>
                          <option value="from-indigo-600 to-blue-700">Indigo</option>
